...
class SpadesRuleBasedAgent:
    ...
    def select_action(self, obs: np.ndarray, legal_moves: List[int]) -> int:
        ...
        trump_oh = v[156:160]
        trump = int(np.argmax(trump_oh)) if float(trump_oh.sum()) > 0.5 else 2  # default Spades
        ...
        pos_in_trick = len(table_cards)  # 1..3
        ...
        if lead_suit is not None and any(self._suit(c) == lead_suit for c in legal_moves):
        ...
        if trumps_in_legal:
            if current_winner is not None and self._suit(current_winner) == trump:
                winning_trumps = [c for c in trumps_in_legal if self._rank_key(c) > self._rank_key(current_winner)]
                if winning_trumps:
                    if pos_in_trick == 3 or self._count_suit(hand, trump) >= 4:
                        return min(winning_trumps, key=self._rank_key)
                    low_win = [c for c in winning_trumps if self._rank_key(c) <= 4]  # 2..6 civarı
                    if low_win:
                        return min(low_win, key=self._rank_key)
                    return self._discard_move(hand, legal_moves, trump)

                return self._discard_move(hand, legal_moves, trump)

            if pos_in_trick == 3:
                return min(trumps_in_legal, key=self._rank_key)

            trump_count = self._count_suit(hand, trump)
            if trump_count >= 5:
                return min(trumps_in_legal, key=self._rank_key)

            low_trumps = [c for c in trumps_in_legal if self._rank_key(c) <= 4]  # 2..6
            if low_trumps:
                return min(low_trumps, key=self._rank_key)
    ...
    def _lead_move(self, hand: List[int], legal_moves: List[int], trump: int) -> int:
        ...
        if non_trump_suits:
            best_suit = max(
                non_trump_suits,
                key=lambda s: (suit_counts[s], self._max_rank_in_suit(hand, s))
            )
        ...
        if trumps:
            ...
            sorted_tr = sorted(trumps, key=self._rank_key)
            return sorted_tr[len(sorted_tr) // 2]
    ...
    def _discard_move(self, hand: List[int], legal_moves: List[int], trump: int) -> int:
        ...
        if non_trumps:
            return max(non_trumps, key=lambda c: (-self._rank_key(c), -1.0 / (suit_counts.get(self._suit(c), 1))))
    ...
    def _count_suit(self, hand: List[int], suit: int) -> int:
        return sum(1 for c in hand if self._suit(c) == suit)
